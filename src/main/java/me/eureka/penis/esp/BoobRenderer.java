...
public class BoobRenderer extends Module {

    public BoobRenderer() {
        super(Categories.Render, "Boob Renderer", "gives you La Teta");
    }
    ...
    public Setting<Double> ballsSize = sgGeneral.add(new DoubleSetting.Builder().name("boobs-size").defaultValue(0.1).range(0.1, 0.5).sliderRange(0.1, 0.5).build());
    ...
    public Setting<SettingColor> penisColor = sgRender.add(new ColorSetting.Builder().name("BooB-color").defaultValue(new SettingColor(170, 90, 0,153)).build());
    ...
    public void onRender(Render3DEvent event) {
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            Vec3d forward = base.add(0, player.getHeight() / 1.5, 0).add(Vec3d.fromPolar(0, player.getYaw()).multiply(0.1));
